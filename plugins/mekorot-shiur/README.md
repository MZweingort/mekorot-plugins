...
## מה בתוך התוסף
- **פקודה `/mekorot`** – קיצור: `/mekorot` (עם או בלי נתיב לקובץ) מפעיל את כל התהליך.
...
**ב-Cowork:** שיחה חדשה → לצרף את קובץ ה-docx → להקליד **`/mekorot`** (או לכתוב "מלא מקורות בהערות הריקות") → הקובץ `... - עם מקורות.docx` חוזר בשיחה.

**ב-Claude Code:** להריץ `claude` בתיקייה שבה נמצא השיעור, ולהקליד **`/mekorot shiur.docx`** (או בלי נתיב, אם יש רק שיעור אחד בתיקייה). הקובץ החדש נכתב לצד המקור באותה תיקייה.
